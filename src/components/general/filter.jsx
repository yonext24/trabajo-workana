...
export function Filter () {
  const { general: { dependencias: { data: { complete } }, sectores: { data: sectoresData } } } = useSelector(s => s.data)
  ...
  const options = useMemo(() => {
    return ['all'].concat(sectoresData)
  }, [complete])

  const handleChange = (value) => {
    if (value === 'all') setGeneralDependenciesFilteredData(complete)
    else setGeneralDependenciesFilteredData(complete.filter(el => el.sector.toLowerCase() === value.toLowerCase()))
  }
