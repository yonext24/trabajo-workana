...
export function ButtonsContainer ({ children, closeModal, alone, className, disabled }) {
  ...
  [&>button]:w-full [&>button]:max-w-[140px] [&>button]:sm:max-w-[200px] [&>button]:rounded-md [&>button]:text-md [&>button]:sm:text-button ${className ?? ''}`}>
    {children}
    {
      !alone &&
      <button type='button' disabled={disabled} onClick={closeModal}>Salir</button>
    }
