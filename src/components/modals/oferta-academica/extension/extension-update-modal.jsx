  ...
  return <ModalBackground onClick={closeModal} closeModal={closeModal}>
    <DefaultModalLayout title={'Agregar extensión'} className={'!max-h-[calc(100vh_-_5px)]'}>
      <form className='p-6 gap-4 flex flex-col [&_label]:text-lg [&_label]:font-semibold overflow-y-auto' onSubmit={handleSubmit(handleUpdate)} >
        ...
        <InputWLabel defaultValue={codigo} id={'codigo'} name='codigo' register={register} labelText={'Código'} required />
        ...
        <div className='flex flex-col w-full'>
          ...
          <SelectInputControlled disabled control={control} name='unidad' options={['México', 'Guatemala', 'El Salvador', 'Estados Unidos']} rules={{ required: true }} />
        </div>
        <InputWLabel type='date' register={register} id='date' name='date' required labelText={'Fecha de creación'} defaultValue={fecha_de_creacion} />
        <div className='flex flex-col w-full'>
          <label>Estado</label>
          <SelectInputControlled defaultValue={estado} disabled control={control} name='unidad' options={['Activo', 'Desactivado']} rules={{ required: true }} />
        </div>
