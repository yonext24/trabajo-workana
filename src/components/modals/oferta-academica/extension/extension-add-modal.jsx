...
export function ExtensionAddModal ({ closeModal }) {
  ...
  const handleUpdate = data => {
    ...
    addOfertaAcademicaExtension({ ...data, estado: 'Activo' })
      .then(closeModal)
  }
  ...
  return <ModalBackground onClick={closeModal} closeModal={closeModal}>
    <DefaultModalLayout title={'Agregar extensión'} className={'!max-h-[calc(100vh_-_5px)]'}>
      <form className='p-6 gap-4 flex flex-col [&_label]:text-lg [&_label]:font-semibold overflow-y-auto' onSubmit={handleSubmit(handleUpdate)} >

        <div className='flex gap-4'>
          <div className='flex flex-col w-full'>
            <label>Tipo Unidad</label>
            <SelectInputControlled tabindex='0' control={control} name={'tipo'} options={['1', '2', '3', '4']} rules={{ required: true }} />
          </div>
          <div className='flex flex-col w-full'>
            <label>Unidad</label>
            <SelectInputControlled control={control} name='unidad' options={['1', '2', '3', '4']} rules={{ required: true }} />
          </div>
        </div>
        ...
        <div className='flex flex-col w-full'>
          ...
          <SelectInputControlled control={control} name='unidad' options={['México', 'Guatemala', 'El Salvador', 'Estados Unidos']} rules={{ required: true }} />
        </div>
        <InputWLabel type='date' register={register} id='fecha_de_creacion' name='fecha_de_creacion' required labelText={'Fecha de creación'} defaultValue={new Date().toISOString().split('T')[0]} />
