...
export function UnidadAddModal ({ closeModal }) {
  ...
  const handleUpdate = (newData) => {
    ...
    addUnidadAcademicaUnidad(newData)
      .then(closeModal)
  }
    ...
    <DefaultModalLayout title='Agregar Unidad'>
      <form onSubmit={handleSubmit(handleUpdate)} className='px-8 py-4 pb-12 flex flex-col gap-y-3'>
        <div className='flex flex-col'>
          ...
          <SelectInputControlled control={control} name='tipo' rules={{ required: true }} options={['Escuela', 'Test']} />
        </div>
        <InputWLabel name='codigo' id='codigo' labelText='Código' required register={register} />
        <InputWLabel name='nombre' id='nombre' labelText='Nombre' required register={register} />
