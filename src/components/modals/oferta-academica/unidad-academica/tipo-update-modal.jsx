...
export function TipoUpdateModal ({ closeModal, nombre, descripcion }) {
  ...
  const handleUpdate = (data) => {
    if (tiposData.some(el => el.nombre === data.nombre && data.nombre !== nombre)) {
  ...
  return <ModalBackground onClick={closeModal} closeModal={closeModal}>
    <DefaultModalLayout title='Agregar Tipo'>
      <form className='p-6 flex flex-col gap-3' onSubmit={handleSubmit(handleUpdate)}>
        ...
        <InputWLabel defaultValue={nombre} name='nombre' id='nombre' register={register} type='text' required />
        <InputWLabel defaultValue={descripcion} name='descripcion' id='descripcion' register={register} type='text' required inputClassName='mb-12' isTextArea rows={3} />
