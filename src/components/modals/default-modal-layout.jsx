...
export function DefaultModalLayout ({ title, className, children, closeModal = false }) {
  return <div className={`border-2 border-black flex flex-col w-full bg-white max-w-xl ${className ?? ''}`} onClick={e => e.stopPropagation()}>
    <header className="bg-azulfondo text-white py-1 text-center border-b-2 border-black relative">
      ...
      <button onClick={() => { closeModal && closeModal() }} className="rounded-full p-px absolute right-1 top-1/2 -translate-y-1/2 border-2 border-white text-white">
