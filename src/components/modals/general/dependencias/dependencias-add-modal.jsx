...
export function DependenciasAddModal ({ closeModal }) {
  ...
  const onSubmit = handleLoading(async (data) => {
    ...
    await addDependenciasData({ ...data, sector: sector.nombre, unidad: unidad.nombre, id_unidad, id_sector })
    closeModal()
  })
  ...
  return <ModalBackground closeModal={closeModal} onClick={closeModal}>
    ...
    <DefaultModalLayout title='Agregar Sector' closeModal={closeModal} loading={loading} errors={errors}>
      <form onSubmit={handleSubmit(onSubmit)} className='py-8 px-4 font-semibold flex flex-col gap-y-4'>
        ...
        <InputWLabel id='nombre' labelText='Nombre' type='text' autoFocus register={register} required registerProps={{
          validate: (nombre) => {
            if (dependenciasData.some(d => d.nombre === nombre)) return 'Ya existe una dependencia con ese nombre'
          }
        }} />
        <InputWLabel id='abreviatura' name='abreviatura' labelText='Abreviatura' type='text' register={register} required registerProps={{
          validate: (abreviatura) => {
            if (dependenciasData.some(d => d.abreviatura === abreviatura)) return 'Ya existe una dependencia con esa abreviatura'
          }
        }} />
