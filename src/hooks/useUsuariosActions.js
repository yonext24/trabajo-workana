...
import { add_permission, add_role, delete_permission, delete_role, get_permisos_data, get_role_permissions, get_roles_data, update_permission } from '@/store/usuarios/thunks'
import { useDispatch } from 'react-redux'
export function useUsuariosActions () {
  const dispatch = useDispatch()

  const getRolesData = async () => { dispatch(get_roles_data()) }
  const getRolePermissions = async (role) => { return dispatch(get_role_permissions({ role })) }
